// ==UserScript==
// @name         openplace: li'l info bubble
...
// @downloadURL https://update.greasyfork.org/scripts/553016/openplace%3A%20li%27l%20info%20bubble.user.js
// @updateURL https://update.greasyfork.org/scripts/553016/openplace%3A%20li%27l%20info%20bubble.meta.js
// ==/UserScript==
...
async function refreshData() {
	"use strict";
	const response = await fetch("https://openplace.live/me", {credentials: "include"});
	if (!response.ok) throw "Fetching data failed";
	const responseTime = Date.parse(response.headers.get("Date")) || Date.now();
	const userData = await response.json();
	const msToCap = (userData.charges.max - userData.charges.count) * 2 * 1000;
