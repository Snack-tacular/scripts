// ==UserScript==
// @name         Openplace Overlay Pro Modified By @SrCratier (English)
  ...
  <div class="op-header" id="op-header">
        ...
        <div class="op-tab-panes op-section" style="padding-top: 12px; border-top-left-radius: 0;">
            <div class="op-tab-pane active" data-pane="overlays">
                <div class="op-row space">
                    <button class="op-button" id="op-add-overlay" title="Create new overlay">+ Add</button>
                    ...
                    <div>
                        <div class="op-row">
                            ...
                            <select class="op-select op-grow" id="op-color-mode" title="Choose how colors are processed.">
                                ...
                                <option value="hsv">HEAVY!💀(Color Priority/Anime)</option>
                            </select>
                        </div>
                    </div>
                    <div>
                        <div id="op-image-source">
                            <div class="op-row">
                                ...
                                <input type="text" class="op-input op-grow" id="op-image-url" placeholder="Paste image link">
            ...
            <div class="op-tab-pane" data-pane="tools">
                <div>
                    ...
                    <div class="op-row space" style="margin-top: 4px;">
                        <button class="op-button" id="op-copy-preview-toggle" style="flex:1;">Preview Area</button>
                        <button class="op-button" id="op-copy-create" style="flex:1;" title="Detect and download an image of the selected area.">Detect & Download</button>
                    </div>
                </div>
    ...
    settingsModal.innerHTML = `
        ...
        <div class="op-donation-section">
            <p>This project is free, but a donation to support development would be appreciated ❤️</p>
    ...
    <div class="op-ca-filters-pane" id="op-ca-filters-pane">
        <div class="op-ca-filter-actions">
            ...
            <button class="op-button" id="op-ca-mark-all">Check all</button>
            <button class="op-button" id="op-ca-mark-none">Uncheck</button>
...
`;
    ...
    caSettingsModal.innerHTML = `
        ...
        <div class="op-donation-section">
            <p>This project is free, but a donation to support the project would be appreciated ❤️</p>
            <div class="op-donation-info">
                <span>Binance ID:</span>
                <code>851390091</code>
            </div>
...
function rebuildOverlayListUI() {
  ...
  for (const ov of config.overlays) {
    ...
    trashBtn.addEventListener('click', async (e) => {
      e.stopPropagation();
      if (!confirm(`Delete overlay "${ov.name || '(no name)'}"?`)) return;
...
async function processImageToPalette(base64, mode = 'perceptual') {
  ...
  async function setOverlayImageFromURL(ov, url) {
    ...
    showToast(`Image processed and loaded. Click to set anchor.`);
  }
...
async function setOverlayImageFromFile(ov, file) {
    ...
    showToast(`Local image processed. Click to set anchor.`);
  }
  ...
  async function createCanvasCopy() {
    ...
    if (config.showOverlay && config.overlayMode !== 'original') {
        showToast('Notice: Copying original canvas without overlay.', 4000);
    }
    ...
    if (W > 50000 || H > 50000) {
        showToast(`Area too large (${W}x${H}). Max is 50000px per side.`);
        return;
    }
    ...
    if (missingTiles.length > 0) {
        showToast(`Incomplete area. Please move map over the selected zone to load data and try again.`);
        console.log("Missing the following tiles:", missingTiles);
        return;
    }
  ...
  async function nudgeCopyPoint(dx, dy) {
      ...
      if (!point) {
          showToast(`Point ${config.copyNudgeTarget} is not set.`);
          return;
      }
      point.absX += dx;
      point.absY += dy;
...
applyTheme();
    ...
    $('op-fetch').addEventListener('click', async () => {
        ...
        try { await setOverlayImageFromURL(ov, url); } catch (e) { console.error(e); alert('Could not load image.'); }
    });
    ...
    $('op-file-input').addEventListener('change', async (e) => {
        ...
        const ov = getActiveOverlay(); if (!ov) return;
        if (ov.imageBase64) { alert('This overlay already has an image. Create a new one to change it.'); return; }
        try { await setOverlayImageFromFile(ov, file); } catch (err) { console.error(err); alert('Could not load local image.'); }
    });
    ...
    dropzone.addEventListener('drop', async (e) => {
        ...
        const ov = getActiveOverlay(); if (!ov) return;
        if (ov.imageBase64) { alert('This overlay already has an image. Create a new one to change it.'); return; }
        try { await setOverlayImageFromFile(ov, file); } catch (err) { console.error(err); alert('Could not load dropped image.'); }
    });
    ...
    const setCopyPoint = (point) => {
        ...
        showToast(`Click canvas to set point ${point}`);
        updateUI();
        ensureHook();
    };
...
async function updateOverlayProgress() {
    ...
    if (!ov || !ov.imageBase64 || !ov.pixelUrl) {
        panelContent.innerHTML = `<span class="op-muted" style="text-align: center; padding: 20px 0;">Select an overlay with image and anchor set.</span>`;
    ...
    try {
        ...
        if (colorData.size === 0) {
            panelContent.innerHTML = `<span class="op-muted" style="text-align: center; padding: 20px 0;">Image is empty.</span>`;
            return;
        }
  ...
  function updateEditorUI() {
    ...
    $('op-coord-display').textContent = ov.pixelUrl
      ...
      : `No anchor set. Enable "Set Position" and click a pixel.`;
  ...
  function updateCopierUI() {
    ...
    if (canCreate) {
        ...
        info.textContent = `Size selected: ${W} x ${H} pixels.`;
    } else {
...
function updateUI() {
  ...
  function buildCCModal() {
    ...
    modal.innerHTML = `
      <div class="op-cc-header" id="op-cc-header">
        ...
        <div class="op-row" style="gap:6px;">
          <button class="op-button op-cc-pill" id="op-cc-realtime" title="Toggle real-time calculation when changing palette.">Live: OFF</button>
        ...
        <div class="op-cc-controls" style="grid-area: controls;">
          <div class="op-cc-palette" id="op-cc-free">
            <div class="op-row space">
              ...
              <button class="op-button" id="op-cc-free-toggle" title="Select/Deselect all colors in this palette.">Uncheck All</button>
            </div>
          ...
          <div class="op-cc-palette" id="op-cc-paid">
            <div class="op-row space">
              ...
              <button class="op-button" id="op-cc-paid-toggle" title="Select/Deselect all colors in this palette.">Check All</button>
            </div>
    ...
    cc.applyBtn.addEventListener('click', async () => {
      ...
      if (cc.processedCanvas.width >= MAX_OVERLAY_DIM || cc.processedCanvas.height >= MAX_OVERLAY_DIM) {
        showToast(`Image is too large to apply (must be < ${MAX_OVERLAY_DIM}×${MAX_OVERLAY_DIM}).`); return;
      }
    ...
    function markStale() {
      cc.isStale = true;
      cc.meta.textContent = cc.meta.textContent.replace(/ \| Status: .+$/, '') + ' | Status: pending calculation';
    }
    function recalcNow() {
      processImage();
      cc.isStale = false;
  ...
  function updateMeta() {
    ...
    const status = cc.isStale ? 'pending calculation' : 'updated';
  ...
  function buildRSModal() {
    ...
    modal.innerHTML = `
      ...
      <div class="op-rs-body">
        ...
        <div class="op-rs-pane" id="op-rs-pane-advanced">
          ...
          <div class="op-rs-row">
            ...
            <label for="op-rs-bind">Bind block sizes X/Y</label>
          </div>
          ...
          <div class="op-rs-grid-note" id="op-rs-adv-note">Align red dots with block centers. Drag to move; use buttons or Ctrl+wheel to zoom.</div>
      ...
      <div class="op-rs-footer">
        ...
        <div class="op-cc-actions">
          <button class="op-button" id="op-rs-calc" title="Calculate result preview with advanced parameters.">Calculate</button>
          <button class="op-button" id="op-rs-apply" title="Apply size changes to overlay image.">Apply</button>
    ...
    const computeSimpleFooterText = () => {
      ...
      return ok ? (limit ? `Target: ${W}×${H} (exceeds limit: < ${MAX_OVERLAY_DIM}×${MAX_OVERLAY_DIM})` : `Target: ${W}×${H} (OK)`) : 'Enter positive width and height.';
    };
    const sampleDims = () => {
    ...
    const computeAdvancedFooterText = () => {
      ...
      return (cols>0 && rows>0) ? `Samples: ${cols} × ${rows} | Output: ${cols}×${rows}${limit ? ` (exceeds limit: < ${MAX_OVERLAY_DIM}×${MAX_OVERLAY_DIM})` : ''}` : 'Adjust multiplier/offset until dots are centered.';
    };
    ...
    const syncSimpleNote = () => {
      ...
      const ok = Number.isFinite(W) && Number.isFinite(H) && W>0 && H>0;
      const limit = (W >= MAX_OVERLAY_DIM || H >= MAX_OVERLAY_DIM);
      const simpleText = ok ? (limit ? `Target: ${W}×${H} (exceeds limit: < ${MAX_OVERLAY_DIM}×${MAX_OVERLAY_DIM})` : `Target: ${W}×${H} (OK)`) : 'Enter positive width and height.';
    ...
    rs.applyScale.addEventListener('click', () => {
      ...
      if (!Number.isFinite(s) || s<=0) { showToast('Enter valid scale factor > 0'); return; }
    ...
    rs.calcBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        showToast(`Calculated ${cols}×${rows}. Check preview then Apply.`);
    ...
    img.onload = () => {
      ...
      const setFooterNow = () => {
        if (rs.mode === 'advanced') {
          ...
          rs.meta.textContent = (cols>0&&rows>0) ? `Samples: ${cols} × ${rows} | Output: ${cols}×${rows}${(cols>=MAX_OVERLAY_DIM||rows>=MAX_OVERLAY_DIM)?` (exceeds limit: < ${MAX_OVERLAY_DIM}×${MAX_OVERLAY_DIM})`:''}` : 'Adjust multiplier/offset until dots are centered.';
        } else {
          const W = parseInt(rs.w.value||'0',10); const H = parseInt(rs.h.value||'0',10);
          const ok = Number.isFinite(W)&&Number.isFinite(H)&&W>0&&H>0;
          const limit = (W>=MAX_OVERLAY_DIM||H>=MAX_OVERLAY_DIM);
          rs.meta.textContent = ok ? (limit ? `Target: ${W}×${H} (exceeds limit: < ${MAX_OVERLAY_DIM}×${MAX_OVERLAY_DIM})` : `Target: ${W}×${H} (OK)`) : 'Enter positive width and height.';
        }
      };
      setFooterNow();
      const onResize = () => {
